...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($appointmentId > 0) {
        db()
            ->prepare(
                "INSERT IGNORE INTO notification_reads(user_id,appointment_id) VALUES(?,?)",
            )
            ->execute([$userId, $appointmentId]);
    }
    echo json_encode(["ok" => true]);
    exit();
}
$s = db()
    ->prepare("SELECT a.id,c.name client,s.name service,COALESCE(b.name,'Sem profissional') barber,a.start_at
...
foreach ($s as $r) {
    $items[] = [
        ...
        "service" => $r["service"],
        "barber" => $r["barber"],
        "when" => date("d/m/Y H:i", strtotime($r["start_at"])),
    ];
}
echo json_encode(
